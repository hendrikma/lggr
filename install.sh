...
echo "[LGGR] Starting install-script"
...
then
  echo "[LGGR] BasicAuth set to 'false'. Uncommenting lines in lggr_class.php"
  sed -i '52,54 s/^/#/' "$DIR/inc/lggr_class.php"
else
  echo "[LGGR] BasicAuth set to 'true'"
  sed -i '52,54 s/^#//' "$DIR/inc/lggr_class.php"
...
then
  ...
  echo "[LGGR] Set Syslg-ng option 'no-caps'"
fi
#--------------------------------------------------------------------------------------------
function init_mysql
{
...
then
	echo "[LGGR] Service MySQL not running. Starting ..."
...
echo "[LGGR] Checking requirements ..."
if [ ! "$(PKG_OK "apache2")" ]
then
echo "[LGGR] Service apache2 not installed. Installing now ..."
PKG_INSTALL="${PKG_INSTALL} apache2"
fi

if [ ! "$(PKG_OK "php*")" ]
then
echo "[LGGR] Service PHP not installed. Installing now ..."
PKG_INSTALL="${PKG_INSTALL} php7.3 php-mysql php-redis"
fi
...
echo "[LGGR] Set php$php_version timezone to $PHP_TIMEZONE"

if [ ! "$(PKG_OK "redis-server")" ]
then
echo "[LGGR] Service Redis-server not installed. Installing now ..."
...
then
	...
	echo "[LGGR] Skipping syslog configuration. $SYSLOG_CONF already exists"
fi
#--------------------------------------------------------------------------------------------

echo "[LGGR] Restarting services ..."
service apache2 restart && service mysql restart && service redis-server restart && service syslog-ng restart && service cron restart
#supervisord # Under construction

echo "[LGGR] End of install-script"
